...
tinyMCE.init({
    ...
    setup : function(ed) {
            var max = 10;
                ed.onKeyDown.add(function(ed, l) {
                    var mensaje = tinyMCE.activeEditor.getContent({format : 'text'})
                    if(mensaje.length >= max){
                        var cortar = mensaje.substr(1,mensaje.length)
                        console.log(cortar)
                        tinyMCE.activeEditor.setContent(cortar)
                    }
              });
    },
    // Content CSS
    ...
    theme_advanced_resizing_max_width: '1000',
